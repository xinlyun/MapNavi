	...
	public static void onNotifyCallback(int funcid, byte[] data) {
		...
		if(funcid==1) {
			try {
				...
				JSONObject wsPoi = service.getJSONObject("wxmsg_poi");
				double lat = wsPoi.getDouble("latitude");
				double lot = wsPoi.getDouble("longitude");
				String name = wsPoi.getString("label");
				Intent intent = new Intent();
				intent.setAction(LocationProService.ACTION_NAVI2);
				intent.putExtra("lat",(float)lat);
				intent.putExtra("lot",(float)lot);
				intent.putExtra("name",name);
				XpApplication.sApplication.sendBroadcast(intent);
			...
			try {
				JSONObject jsonObject = new JSONObject(dataString);
				JSONObject service = jsonObject.getJSONObject("services");
				JSONObject wsPoi = service.getJSONObject("wxmsg_music");
//				double lat = wsPoi.getDouble("latitude");
//				double lot = wsPoi.getDouble("longitude");
//				String name = wsPoi.getString("label");
//				Intent intent = new Intent();
//				intent.setAction(NaviProService.ACTION_NAVI2);
//				intent.putExtra("lat",(float)lat);
//				intent.putExtra("lot",(float)lot);
//				intent.putExtra("name",name);
//				CarControlApplication.sApplication.sendBroadcast(intent);
			} catch (JSONException e) {
				e.printStackTrace();
			}
